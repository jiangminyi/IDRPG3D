...
namespace InsaneSystems.RTSStarterKit
{
	public class Attackable : Module
	{
		...
		public event AttackAction startAttackEvent, stopAttackEvent;
		...
		readonly Collider[] objectsInSearchRadius = new Collider[500];
		...
		protected virtual void SearchForTarget()
		{
			...
			for (int i = 0; i < objectsCount; i++)
			{
				...
				if (selfUnit.IsInMyTeam(unit) || unit == selfUnit || !unit.GetModule<FogOfWarModule>().isVisibleInFOW)
					continue;

				var order = new AttackOrder();
				order.attackTarget = unit;
		...
		void DoShoot()
		{
			var spawnedObject = Instantiate(selfUnit.data.attackShell, shootPoints[currentShootPoint].position, shootPoints[currentShootPoint].rotation);
			var shell = spawnedObject.GetComponent<Shell>();
			shell.SetOwner(selfUnit.OwnerPlayerId);
			shell.SetTarget(attackTarget.transform);
